...
def test_the_experiment_does_not_join_the_fourteen_comparable_levels() -> None:
    """La tarjeta del experimento se ofrece, pero NO entra en `levels`.

    Son catorce cargas comparables entre sí; una decimoquinta que no está en el YAML y
    que ni siquiera corre el mismo entrypoint las dejaría de serlo.
    """
    catalogue = _catalogue()
    experiment = next(item for item in catalogue if item["key"] == "orient-by-com")

    assert len([item for item in catalogue if item["kind"] == "level"]) == 14
