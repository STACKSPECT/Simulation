...
`scripts/palletize.py` is the palletizing entry point. The separate CoM-orientation experiment
uses `scripts/orient_by_com.py` as described above.
...
| 1 | `python tests/test_pallet.py` | Row keys are columns, `seq` never repeats, vocabularies hold, the adapter's silent unit translations | `28 comprobaciones pasadas` |
| 2 | `python -m src.measure` | CoG with out-of-tolerance boxes, margin against the support polygon | `ok measure.demo` |
| 3 | `python tests/test_cell.py` | Starts MuJoCo: three sources, cameras, belt, truck order, IK envelope, wrist gauge and CoM reorientation | `18 comprobaciones físicas pasadas` |
